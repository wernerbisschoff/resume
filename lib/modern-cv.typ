...
#let __format_contact_items(author, item-inset: (:)) = {
  ...
  if "linkedin" in author {
    items.push(
      contact-item(
        (
          text: "linkedin.com/in/" + author.linkedin,
          icon: linkedin-icon,
...
#let resume-item(body) = {
  set text(size: 10pt, style: "normal", fill: color-darknight)
...
#let default-signature(lang-data, language, author, alignment, padding) = {
  align(alignment, pad(..padding)[
    #text()[#linguify("sincerely", from: lang-data)#if (
        language != "de"
      ) [#sym.comma]] \
...
#let coverletter(
  ...
  let contacts = {
    set text(size: 8pt, style: "normal")
