    ...
    images.map(async (image) => {
      if (image?.url) {
        ...
        if (
          ...
          // Use config-provided dimensions first, then fall back to image metadata, then defaults
          const configWidth = typeof image?.width === 'number' ? image.width : undefined;
          const configHeight = typeof image?.height === 'number' ? image.height : undefined;

          const dimensions =
            typeof resolvedImage !== 'string' && resolvedImage?.width
              ? [resolvedImage?.width, resolvedImage?.height]
              : [configWidth || defaultWidth, configHeight || defaultHeight];
