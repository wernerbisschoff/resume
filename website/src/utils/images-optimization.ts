...
/* ******* */
const config = {
  // imageSizes for thumbnails and small images (icons, avatars)
  // Currently unused - all images use deviceSizes breakpoints
  // Reserved for future use with small images to avoid unnecessary large responsive variants
...
export const astroAssetsOptimizer: ImagesOptimizer = async (
  ...
  return Promise.all(
    breakpoints.map(async (w: number) => {
      // Calculate height proportionally if width and height are provided
      const h = _width && _height ? Math.floor(w * (_height / _width)) : _height;
