...
draft: false
excerpt: 'I built DeviaTDD to find out whether you can keep AI agents honest through a full TDD cycle. The framework I ended up with rests on a conditional test-amendment phase called Yellow that I have never seen documented anywhere else — and that is the part I am most uncertain about.'
category: DeviaTDD
tags:
  - deviatdd
  - tdd
...
**DeviaTDD is the framework I built to address that — a spec-first, agent-agnostic system that runs the entire TDD cycle (explore, spec, red, green, refactor) with three non-bypassable human gates and a conditional test-amendment phase called Yellow that distinguishes agent cheating from a genuine spec error.**

It's MIT-licensed, written in Python, and ships as a single CLI. The first-commit install is `uv tool install deviate`; the first slash command is `/deviate-explore "..."`. Claude Code, OpenCode, Pi, and Droid are first-class backends.
...
The general-purpose mitigation is "be more careful." My approach is structural: each phase is a non-bypassable artifact transition, and each transition is gated on a check the agent cannot fake.

## The framework: four layers, three gates, one micro-loop

I decompose each feature into a chain of artifacts, one per commit. The chain is not a checklist; it is a routing problem — each phase uses a different model tier and produces a distinct form of evidence.
...
The framework is open source under MIT and lives at [github.com/wbisschoff13/deviatdd](https://github.com/wbisschoff13/deviatdd). Yellow is a single slash command: `/deviate-yellow <task-id>`. If you have run a TDD-style agent loop on real code, I would like to know whether Yellow actually distinguishes cheating from a real spec error — or whether I have just moved the cheat-detection problem one level up. Issues, PRs, and adversarial examples of test-hacking are welcome.
